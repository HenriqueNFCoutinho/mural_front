...
import { buscarAnuncio, criarAnuncio } from "../services/api";
...
export default function EditarAnuncio() {
  ...
  useEffect(() => {
    // TODO: trocar por fetch real quando backend estiver pronto
    // buscarAnuncio(id)
    //   .then(data => setForm({
    ...
    //     preco: data.preco,
    //     tipo_preco: data.tipo_preco || "hora",
    //   }))
    ...
    setTimeout(() => {
      setForm({
        categoria_id: "1",
        titulo: "Monitoria de Cálculo I e II",
        descricao: "Aluno de eng. civil com 2 anos de experiência.",
        preco: "35",
        tipo_preco: "hora",
      });
      setCarregando(false);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // await fetch(`http://localhost:5000/anuncio/atualizar/${id}`, {
      //   method: "POST",
    ...
    return (
      <div className="criar-root">
        <div style={{ display: "flex", justifyContent: "center", padding: "80px" }}>
          <div className="spinner" style={{ width: 28, height: 28, borderWidth: 3, borderColor: "#EDE0D4", borderTopColor: "#C45A10", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
        </div>
      </div>
    );
  }
        ...
        <form className="criar-form" onSubmit={handleSubmit}>
          ...
          <div className="field-float">
            <input
              ...
              placeholder=" "
            />
            <label htmlFor="titulo-edit">Título do anúncio <span className="obrigatorio">*</span></label>
                ...
                <div className="preview-top">
                  ...
                  {form.preco && (
                    <span className="preview-preco">
                      R$ {form.preco}
                      <span className="preview-tipo">/{TIPOS_PRECO.find(t => t.value === form.tipo_preco)?.value}</span>
                    </span>
                  )}
                </div>
